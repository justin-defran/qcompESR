...
void loop() {
    user_input = Serial.read(); //first byte of serial incoming data (which is the first letter)
    if (user_input == 's'){
      single_sweep();
    }
    if (user_input == 'f'){
...
//    Serial.println(t);
    ...
    for (k=0;k<s+10;k++){
     ...
     for (k=0;k<s+10;k++){
         ...
         digitalWrite(LED_BUILTIN,LOW);
         delayMicroseconds(1000*t); //this accounts for the reset time we set in the adbasic sketch
